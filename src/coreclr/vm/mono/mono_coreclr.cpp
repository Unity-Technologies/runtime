...
MonoClass * mono_class_from_name(MonoImage *image, const char* name_space, const char *name, bool ignoreCase)
{
    ...
    EX_TRY
    {
        monoClass = g_HostStruct->class_from_name(image, name_space, name, ignoreCase);
    }
    EX_CATCH
    {
        SString sstr;
        GET_EXCEPTION()->GetMessage(sstr);
        unity_log("CoreCLR Exception %d: %s\n", GET_EXCEPTION()->GetType(), sstr.GetUTF8());
    }
